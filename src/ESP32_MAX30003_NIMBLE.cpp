...
#include <cstdlib>
#include <SPI.h>
#include "C:\Users\patelkus\Documents\libraries\ProtoCentral_MAX30003_ECG_AFE_Sensor_Library\src\protocentral_Max30003.h"
// #define INT_PIN 02
// #include <esp_sleep.h>
int w = 0;
MAX30003 max30003;
...
void loop()
{
  ...
  while (deviceConnected)
  {
    ...
    while (myValue > 0 && deviceConnected == true)
    {
      ...
      analogvalue = analogRead(GPIO_BATTERY);
      int analogvalue1 = analogReadMilliVolts(GPIO_BATTERY); 
      // Serial.println("analog value: " + String(analogvalue) + "  voltage: " + String(analogvalue1));

      if (analogvalue1 > 2000)
      {
        analogvalue1 = 2000;
      }

      if (analogvalue1 < 1600)
      {
        analogvalue1 = 1600;
      }

      battery_percentage1 = map(analogvalue1, 1600, 2000, 1, 100);
      // Serial.println(battery_percentage1);
